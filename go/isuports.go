...
func competitionScoreHandler(c echo.Context) error {
	...
	tenantLocker.Lock(v.tenantID)
	defer tenantLocker.Unlock(v.tenantID)
...
func playerHandler(c echo.Context) error {
	...
	// player_scoreを読んでいるときに更新が走ると不整合が起こるのでロックを取得する
	tenantLocker.RLock(v.tenantID)
	defer tenantLocker.RUnlock(v.tenantID)
...
func competitionRankingHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
		"SELECT MAX(row_num) AS max_row_num, score, player_id FROM player_score WHERE tenant_id = ? AND competition_id = ? GROUP BY player_id ORDER BY score LIMIT 110",
		tenant.ID,
		competitionID,
	); err != nil {
